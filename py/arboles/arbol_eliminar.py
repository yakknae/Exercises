...
class ArbolBinario:
    ...
    def main(self):
        ...
        while True:
            mostrarMenu()
            opcion = input("ingrese una opcion del 1 al 9: ")

            if opcion == '1':
                valor = input("Que valor queres agregar: ")
                if valor:
                    valor= int(valor)
                    self.agregar(valor)
                    print(f"Valor: {valor} insertado")
                else:
                    print("no se pudo agregar el valor")
            elif opcion == '2':
                ...
                print("la altura del arbol es: ", self.altura(self.raiz))
            elif opcion == '6':
                print("el factor de balanceo del arbol es: ",self.factor_balanceo(self.raiz))
            elif opcion == '7':
                valor_buscar = input("Ingrese el nodo que quieras buscar: ")
                valor_buscar = int(valor_buscar)
                nodo, padre, ubicacion, h_izq, h_der = self.buscar(valor_buscar)
                if nodo:
                    ...
                    print(f"el nodo: {nodo.valor} No fue encontrado")
            elif opcion == '8':
                valor_buscar = input("Ingresa el nodo que quieren eliminar: ")
                valor_buscar = int(valor_buscar)
                self.eliminar(valor_buscar)
                print(f"despues de eliminar el {valor_buscar}: ")
                nodo, padre, ubicacion, h_izq, h_der = self.buscar(valor_buscar)
                if nodo:
                    print(f"✅ Encontrado: {nodo.valor}")
                    print(f"🔹 Padre: {padre.valor if padre else 'Ninguno (es raíz)'}")
